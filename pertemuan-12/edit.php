...
sembari pengunjung ke halaman awal (read.php) sembari
menampilkan penanda error.
*/
if (!$cid) {
...
// Flash error dan nilai old input kalau ada
$flash_error = $_SESSION['flash_error'] ?? '';
$old = $_SESSION['old'] ?? [];

$old_nama = $old['cnama'] ?? $nama;
$old_email = $old['cemail'] ?? $email;
$old_pesan = $old['cpesan'] ?? $pesan;
}
?>
    ...
    <section id="contact">
        ...
        <?php if (!empty($flash_error)) : ?>
            <div style="padding:10px; margin-bottom:10px;
                background:#ff8d7a; color:#721C24; border-radius:6px;">
                <?= $flash_error; ?>
            </div>
        <?php endif; ?>
        ...
        <form action="proses_update.php" method="POST">
            ...
            <button type="kembali" onclick="window.location.href='read.php'">
                Kembali
            </button>
        </form>
    </section>
